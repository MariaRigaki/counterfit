...
class GenericLGB(ArtTarget):
    model_name = 'genericlgb'
    model_data_type = "pe"
    model_output_classes = ["benign", "malicious"]
    model_input_shape = (1,)
    model_location = "local"
    model_endpoint = os.path.join(
        config.targets_path, 'genericlgb/model.txt.gz')

    sample_input_path = f"{config.targets_path}/genericlgb/malware_samples.zip"
